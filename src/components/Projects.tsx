  ...
  return (
    <div id="projects" className="max-w-[1100px] mx-[4vw] lg:mx-auto pb-20">
      <div className="flex flex-col gap-8 sm:flex-row items-center sm:justify-center sm:gap-[20vw] lg:justify-evenly">
        <h1
          className={`${bebas.className} text-center w-[11rem] text-[3.5rem] border-b-2 border-gray-500`}
        >
          Projects
        </h1>
        ...
        <div className="relative">
          <input
            ...
            className="bg-transparent focus:bg-none py-3 px-5 rounded-2xl shadow-md shadow-zinc-200 focus:outline-none"
      ...
      <ul className="flex flex-wrap gap-4 items-center justify-center xl:justify-start my-6">
        ...
        {status === STATUS.ERROR && (
          <div className="px-5 py-2 text-red-500 ring-1 ring-red-400">
            ERROR.
          </div>
        )}
        {status === STATUS.SUCCESS &&
          result.map((product: any) => {
            return (
              <div
                key={product.id}
                className="duration-1000 delay-150 w-full sm:w-[18rem] md:w-auto flex flex-col gap-4 justify-center rounded-md shadow-md shadow-gray-200 px-4 py-6"
              >
                <Image
                  src={product.img}
                  alt="project-img"
                  width={500}
                  height={500}
                  className="w-full sm:w-[20rem] rounded-md hover:contrast-50 hover:transform hover:ease-in"
                />
                <div className="flex items-center justify-between">
                  <p className="text-[1rem] font-semibold text-black">
                    {product.name}{" "}
                    <span className="font-light">{product.subname}</span>
                  </p>
                ...
                <div className="flex justify-start items-center gap-2 flex-wrap w-[16.5rem] sm:w-[20rem]">
                  {product.techStack.map((tech: any) => (
                    <p
                      key={tech[0]}
                      className="text-[0.85rem] text-zinc-500 leading-[0.85rem]"
                    >
                      {tech}
                    </p>
                  ))}
